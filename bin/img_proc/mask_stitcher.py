...
def stitch_mask_tiles(
    ...
    tiles_cell = tiles['cell']
    tiles_nuc = tiles['nucleus']
    tiles_cell_b = tiles['cell_boundary']
    tiles_nuc_b = tiles['nucleus_boundary']
    raw_tile_groups = [tiles_cell, tiles_nuc, tiles_cell_b, tiles_nuc_b]

    padding = slicer_info["padding"]
    overlap = slicer_info["overlap"]
    y_ntiles = slicer_info["num_tiles"]["y"]
    x_ntiles = slicer_info["num_tiles"]["x"]
    tile_shape = [slicer_info["tile_shape_with_overlap"]["y"],
                  slicer_info["tile_shape_with_overlap"]["x"]]
    dtype = slicer_info["dtype"]
    ...
    stitched_masks = dict(
        cell=reset_imgs[0],
        nucleus=reset_imgs[1],
        cell_boundary=reset_imgs[2],
        nucleus_boundary=reset_imgs[3]
    )
